...
(() => {
  ...
  Player.prototype.update = function (x, y) {
    ...
    const factor = 1 - Math.pow(1 - TOUCH_MOVE_FACTOR, 1);
    originalPlayerUpdate.call(this, this.x + dx * factor, this.y + dy * factor);
  };
  ...
  function formationOffsets(shape, count) {
    ...
    if (shape === 'line') {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const col = i % cols;
        out.push({ x: (col - (Math.min(cols, count - row * cols) - 1) / 2) * gapX, y: row * gapY });
      }
    }
  ...
  // Formations now rotate between several tactical patterns. None of these
  // creates a power-up: protected power-ups are a separate, less frequent event.
  window.makeFormation = function () {
    const t = tier();
    const variants = ['line', 'v', 'diamond', 'wedge'];
    const shape = variants[Math.floor(Math.random() * variants.length)];
    const variantRoll = Math.random();
    const variant = variantRoll < 0.34 ? 'mixed' : variantRoll < 0.57 ? 'assault' : variantRoll < 0.79 ? 'convoy' : 'swarm';
  ...
  // Protect only a minority of spawned power-ups. Regular power-ups remain
  // common so the game no longer feels locked into the same formation.
  ...
  // The touch offset is deliberately applied here rather than by changing
  // mouse/pen coordinates globally, so desktop mouse behaviour stays exact.
