...
(() => {
  ...
  Bullet = class BulletFixed {
    constructor(x, y, vx = 0, vy = -1, damage = 1) {
      this.x = x;
      this.y = y;
      this.vx = vx * S(powerTable[power].speed);
  ...
  function startGameFixed() {
    if (!running) { finalElapsed = 0; originalStartGame(); return; }
    const needsNewFrameLoop = paused;
    resetGameFixed(); running = true; paused = false; startTime = Date.now(); lastFire = 0;
  ...
  function resumeGameFixed() {
    if (!running || !paused) return;
    pausedTotal += Date.now() - pausedAt; pausedAt = 0; paused = false; lastSimulationAt = 0;
    pauseOverlay.classList.add('hidden'); requestAnimationFrame(loop);
  }
  ...
  function updateFixed() {
    ...
    const before = score;
    originalUpdate();
    if (hardMode && score > before) { score += Math.round((score - before) * 0.5); scoreEl.textContent = 'Score: ' + score; }
  }
  update = updateFixed;
  ...
  function readScoresSafe() {
    try {
      ...
      })).sort((a, b) => b.score - a.score || b.time - a.time).slice(0, 10);
    } catch { return []; }
  }
  ...
  function saveScoreFixed() {
    const name = (playerName.value.trim() || 'Player').slice(0, 14); const all = readScoresSafe(); const elapsed = finalElapsed || secsFixed();
    ...
    saveScoreBox.classList.add('hidden'); renderScoresFixed();
  }
  saveScore = saveScoreFixed;

  function showHardModeNotice() {
    const notice = $('hardModeNotice'); notice.textContent = hardMode ? '⚡ HARD MODE — +50% score' : 'HARD MODE OFF'; notice.classList.remove('hidden');
    clearTimeout(showHardModeNotice.timer); showHardModeNotice.timer = setTimeout(() => notice.classList.add('hidden'), 1600);
  }
  ...
  async function toggleGyro() {
    ...
    if (hardMode && !gyroListenerAttached && 'DeviceOrientationEvent' in window) {
      window.addEventListener('deviceorientation', onOrientation, { passive: true }); gyroListenerAttached = true;
    }
  ...
  Player.prototype.draw = function drawOrientedPlayer() {
    ...
    ctx.beginPath(); ctx.moveTo(0, -this.size); ctx.lineTo(-this.size * 0.72, this.size * 0.72); ctx.lineTo(this.size * 0.72, this.size * 0.72); ctx.closePath();
    ctx.strokeStyle = flash ? '#ff3d3d' : '#fff'; ctx.lineWidth = S(2.5); ctx.stroke(); ctx.restore();
  };
  ...
  $('restartFromGameOverBtn').onclick = startGameFixed; $('restartFromScoreboardBtn').onclick = startGameFixed;
  $('newGameBtn').onclick = startGameFixed; $('restartBtn').onclick = startGameFixed; $('restartFromPauseBtn').onclick = startGameFixed;
  $('pauseBtn').onclick = pauseGameFixed; $('resumeBtn').onclick = () => { settingsMenu.classList.add('hidden'); resumeGameFixed(); }; $('resumeFromPauseBtn').onclick = resumeGameFixed;
  $('closeGameOverBtn').onclick = () => { gameOverMenu.classList.add('hidden'); settingsBtn.style.display = 'none'; startBtn.style.display = 'block'; };
  ...
  settingsBtn.onclick = e => { if (originalSettingsClick) originalSettingsClick.call(settingsBtn, e); if (paused) pauseOverlay.classList.add('hidden'); };
})();
