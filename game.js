...
const $ = id => document.getElementById(id),
    ...
    saveScoreBox = $('saveScoreBox'),
    scoreList = $('scoreList');
const pauseOverlay = $('pauseOverlay');
let width, height, cx, cy, scale = 1,
    player, enemies, bullets, enemyBullets, particles, powerups, score = 0,
    lives = 3,
    running = false,
    paused = false,
    startTime, timerId, lastFire = 0,
    targetX, targetY, power = 0,
    slowUntil = 0,
    invulnerableUntil = 0,
    deferredPrompt = null;
const powerTable = Array.from({
    length: 50
}, (_, i) => ({
    shots: 1,
    cooldown: Math.max(42, 126 - i * 1.7),
    speed: 13 + i * .12,
    spread: Math.min(.48, .09 + i * .008),
    side: i >= 2,
    wide: i >= 7,
    heavy: i >= 10,
    rapid: i >= 16,
...
function resize() {
    ...
    cx = width / 2;
    cy = height / 2;
    scale = Math.max(.78, Math.min(1.18, Math.min(width, height) / 390))
}
...
function secs() {
    return running ? Math.floor((Date.now() - startTime) / 1000) : 0
}

function tier() {
    return Math.floor(secs() / 15)
}
...
class Player {
    ...
    draw() {
        ...
        ctx.lineTo(-this.size * .72, this.size * .72);
        ctx.lineTo(this.size * .72, this.size * .72);
        ctx.closePath();
...
class Enemy{
  constructor(kind='basic'){
    ...
    else if(edge===1){this.x=width+S(32);this.y=Math.random()*height*.7}
    else{this.x=-S(32);this.y=Math.random()*height*.7}
    const a=Math.atan2(height*.58-this.y,cx-this.x),mult=1+tier()*.18;
    this.vx=Math.cos(a)*S((1.22+Math.random()*1.05)*mult);
    this.vy=Math.sin(a)*S((1.22+Math.random()*1.05)*mult);
    this.size=S(kind==='armored'?19:kind==='ghost'?14:12+Math.random()*9);
    this.rot=0;
    this.lastShot=Date.now()+Math.random()*1000;
    // ghost = indistruttibile; armored più resistente col tier
    this.hp=kind==='ghost'?Infinity:kind==='armored'?Math.min(8,3+Math.floor(power/10)):
            (tier()>=6?2:1); // dopo tier 6 anche i basic richiedono 2 colpi
    this.hitUntil=0;
  }
  update(factor){
    this.x+=this.vx*factor;this.y+=this.vy*factor;this.rot+=.04*factor;
    if(this.kind==='shooter'&&Date.now()-this.lastShot>Math.max(330,1200-tier()*34)){
      enemyBullets.push(new EnemyBullet(this.x,this.y+this.size*.8));this.lastShot=Date.now()
    }
  }
  draw(){
    ...
    else if(this.kind==='armored')color='#bd4aff';
    else if(this.kind==='ghost')color='#888888';
    ...
    if(this.kind==='shooter'){ctx.beginPath();ctx.moveTo(0,-this.size*.42);ctx.lineTo(0,this.size*.42);ctx.stroke()}
    if(this.kind==='armored'){ctx.beginPath();ctx.arc(0,0,this.size*.55,0,Math.PI*2);ctx.stroke()}
    if(this.kind==='ghost'){
      // croce al centro per indicare immunità
      ctx.beginPath();ctx.moveTo(-this.size*.35,0);ctx.lineTo(this.size*.35,0);ctx.stroke();
      ctx.beginPath();ctx.moveTo(0,-this.size*.35);ctx.lineTo(0,this.size*.35);ctx.stroke();
    }
    ctx.restore();
  }
...
class Bullet {
    constructor(x, y, vx = 0, vy = -1, damage = 1) {
        ...
        this.y = y;
        this.vx = vx;
        this.vy = vy < 0 ? -S(powerTable[power].speed) : S(powerTable[power].speed);
...
class Particle {
    constructor(x, y, c) {
        ...
        this.life = 1;
        this.d = .025 + Math.random() * .03;
        this.c = c;
        this.r = S(1.5 + Math.random() * 2.5)
    }
    update(f) {
        this.x += this.vx * f;
        this.y += this.vy * f;
...
function chooseEnemy() {
    const t = tier(),
        r = Math.random();
    if (t >= 4 && r < Math.min(.14 + t * .018, .42)) return 'armored';
    if (t >= 2 && r < Math.min(.2 + t * .018, .52)) return 'shooter';
    return 'basic'
}
...
function spawn() {
    const t = tier(),
        cap = 8 + t * 3,
        rate = .032 + t * .006;
    if (enemies.length < cap && Math.random() < rate) enemies.push(new Enemy(chooseEnemy()));
    if (Math.random() < .0017 + Math.min(t, 16) * .00025) powerups.push(new Powerup())
}

function addShot(x, y, vx, vy, damage) {
    bullets.push(new Bullet(x, y, vx, vy, damage))
}
...
function shoot(now) {
    ...
    if (p.side) {
        addShot(player.x, player.y, -S(p.speed), 0, damage);
        addShot(player.x, player.y, S(p.speed), 0, damage)
    }
    if (p.back) {
        addShot(player.x, player.y, 0, 1, damage)
    }
    if (p.wide) {
        addShot(player.x, player.y - player.size, -S(4.4), -1, damage);
        addShot(player.x, player.y - player.size, S(4.4), -1, damage)
    }
    if (p.quad) {
        addShot(player.x, player.y, -S(4.4), -1, damage);
        addShot(player.x, player.y, S(4.4), -1, damage)
    }
}
...
function collisions() {
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        for (let j = bullets.length - 1; j >= 0; j--) {
            if (overlap(eb, bullets[j].b())) {
                bullets.splice(j, 1);
                e.hp--;
                e.hitUntil = Date.now() + 180;
                if (e.hp <= 0) {
                    enemies.splice(i, 1);
                    score += e.kind === 'armored' ? 250 : 100;
                    scoreEl.textContent = 'Score: ' + score;
    ...
    for (let i = enemyBullets.length - 1; i >= 0; i--) {
        if (overlap(pb, enemyBullets[i].b())) {
...
function scores() {
    try {
        return JSON.parse(localStorage.getItem('infinityScores') || '[]')
    } catch {
        return []
    }
}
...
function forceRefresh() {
    if ('caches' in window) caches.keys().then(k => Promise.all(k.map(x => caches.delete(x))).then(() => location.replace(location.pathname + '?update=' + Date.now())));
...
$('showScoresFromGameOverBtn').onclick = () => {
    gameOverMenu.classList.add('hidden');
    renderScores();
    scoreboardMenu.classList.remove('hidden')
};
$('closeScoresBtn').onclick = () => scoreboardMenu.classList.add('hidden');
...
ctx.fillStyle = '#000';
ctx.fillRect(0, 0, width, height);
// NAVIGAZIONE: backToSettings torna sempre al game over solo se running===false
...
// Chiudi scoreboard: stesso comportamento di back
